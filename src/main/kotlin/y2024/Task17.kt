...
//
//		Program: 0,1,5,4,3,0
		var regA = input[0].split(": ")[1].toBigInteger()
		var regB = input[1].split(": ")[1].toBigInteger()
		var regC = input[2].split(": ")[1].toBigInteger()
...
//		for (j in program.indices step 2) {
		...
		while (index < program.size) {
			...
			when (i) {
				0 -> {
					regA = adv(ii, regA, regB, regC).toInt().toBigInteger()
				}
				1 -> {
				...
				3 -> {
					if(regA.not().equals(BigInteger.ZERO)) {
				...
				6 -> {
					regB = bdv(ii, regA, regB, regC).toInt().toBigInteger()
				}
				7 -> {
					regC = cdv(ii, regA, regB, regC).toInt().toBigInteger()
				}
				else -> 0
			}
//			output.add(res)
	...
	private fun cdv(i: Int, regA: BigInteger, regB: BigInteger, regC: BigInteger): BigInteger {
	...
	private fun bdv(i: Int, regA: BigInteger, regB: BigInteger, regC: BigInteger): BigInteger {
	...
	private fun out(i: Int, regA: BigInteger, regB: BigInteger, regC: BigInteger): BigInteger {
		//	The out instruction (opcode 5) calculates the value of its combo operand modulo 8, then outputs that value.
		//	(If a program outputs multiple values, they are separated by commas.)
		return (expandOp(i,regA, regB, regC).mod(BigInteger.valueOf(8L)))
	}

	private fun bxc(i: Int, regA: BigInteger, regB: BigInteger, regC: BigInteger): BigInteger {
	...
	private fun jnz(i: Int, regA: BigInteger, regB: BigInteger, regC: BigInteger): Int {
	...
	private fun bst(i: Int, regA: BigInteger, regB: BigInteger, regC: BigInteger): BigInteger {
		//	The bst instruction (opcode 2) calculates the value of its combo operand modulo 8
		//	(thereby keeping only its lowest 3 bits), then writes that value to the B register.
		return expandOp(i,regA, regB, regC).mod(BigInteger.valueOf(8L))
	}

	private fun bxl(i: Int, regA: BigInteger, regB: BigInteger, regC: BigInteger): BigInteger {
		...
		return regB.xor(i.toBigInteger())
	}

	private fun adv(operand: Int, regA: BigInteger, regB: BigInteger, regC: BigInteger): BigInteger {
...
//to the A register.
		...
		val denominator = BigInteger.valueOf(2.0.pow(op.toDouble()).toLong())
		val result = regA / denominator
		return result
	}

	private fun expandOp(operand: Int, a: BigInteger, b: BigInteger, c: BigInteger): BigInteger {
		return when (operand) {
			4 -> a
			5 -> b
			6 -> c
			else -> operand.toBigInteger()
		}
	}
